## 👋 Hi, I’m Saqib Mukkaram

I’m a systems-focused engineer working across **embedded Linux, networking, and full-stack platforms**.  
My work sits at the intersection of **devices in the field** and **software that keeps them stable, observable, and manageable**.

### 🧠 What I work on
- Embedded Linux & OpenWrt-based router firmware
- Cellular modems
- Device telemetry, monitoring, and remote management
- Backend systems
- Infrastructure with **Docker, Linux servers, and automation**
- Web & app layers when they’re needed to support real systems

I’m most comfortable owning problems end-to-end, especially when documentation stops and systems misbehave in production.

### 🛠️ Tech I regularly use
- **Embedded / Systems:** OpenWrt, ROOter, Linux, MTD/flash layouts
- **Backend:** .NET, Python, REST APIs, Protobuf, C/C++,
- **Infra:** Docker, Nginx, CI/CD, Linux servers
- **Frontend / Apps:** Flutter, React, Next,

### 💼 What I’m open to
- Project-based or contract work
- Embedded / networking problem-solving
- Monitoring and automation
- Collaboration on technically meaningful projects

### 📫 Contact
- Email: **saqibmukkaram@gmail.com**
